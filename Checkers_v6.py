...
class Board:
    ...
    def Click(s):
        ...
        if (X==8 and Y==-3):         #if exit button (in bottom right) is pressed
            ...
            s.CustomSetup()
        elif (4<=X<6 and Y == -3):   #if mouse click is in 'Save' button area
            s.SaveSetupToFile()
        elif (6<=X<8 and Y == -3):   #if mouse click is in 'Load' button area
            s.LoadSetupFromFile()
            ...
            Text(Point(7.5,-1.5),'Del').draw(s.win)
             ...
             ((X%2==1 and Y%2==0) or (X%2==0 and Y%2==1)) or \
             ((s.numWhite() >= 12 and s.PlaceColour == 'White' and s.Place)) or \
             ((s.numBlack() >= 12 and s.PlaceColour == 'Black' and s.Place)):
